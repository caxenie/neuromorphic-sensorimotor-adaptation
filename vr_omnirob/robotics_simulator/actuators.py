...
# dim = joints_num
def joint_positions(cid, handle, val):
    # This function does not use handle, it just exists in the signature
    # to make it consistent
    joint_values = np.zeros(4)
    for i in range(4):
       joint_values[i] = val[i]
    packedData = vrep.simxPackFloats(joint_values.flatten())
    raw_bytes = (ctypes.c_ubyte * len(packedData)).from_buffer_copy(packedData)
    err = vrep.simxSetStringSignal(cid, "moveToPosition",
                                   raw_bytes,
