...
class ControllerWS(tornado.websocket.WebSocketHandler):
    def open(self):
        ...
        if not connected:
            self.bot = show_omnibot.NSTBot()
            self.bot.connect(show_omnibot.Serial('/dev/ttyUSB2', baud=2000000))
            self.omnirob_arm_cam = show_omnibot.NSTBot()
            self.omnirob_arm_cam.connect(show_omnibot.Serial('/dev/ttyUSB3', baud=12000000))
            self.omnirob_arm_cam.retina(True)
            self.omnirob_arm_cam.track_frequencies([1000])

            self.omnirob_left_cam = show_omnibot.NSTBot()
            self.omnirob_left_cam.connect(show_omnibot.Serial('/dev/ttyUSB0', baud=12000000))
            self.omnirob_left_cam.retina(True)
            self.omnirob_left_cam.track_frequencies([1000])

            self.omnirob_right_cam = show_omnibot.NSTBot()
            self.omnirob_right_cam.connect(show_omnibot.Serial('/dev/ttyUSB1', baud=12000000))
