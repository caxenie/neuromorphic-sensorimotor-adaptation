...
class RetinaBot(omnibot.OmniBot):
    def initialize(self):
        ...
        self.sensor_scale = {}
        self.sensor_map = {}
        self.add_sensor('wheel', bit=1, range=1024, length=3)   # -S17 in the sensory stream response
        self.add_sensor('servo', bit=7, range=4096, length=5)   # -S16 in the sensory stream response
        self.add_sensor('load',  bit=9, range=4096, length=5)   # -S14 in the sensory stream response
    ...
    def process_ascii(self, message):
        try:
            ...
            if tokens[0].startswith('-I'):
                tokens = tokens[1:]
            if tokens[0] == '-S17':  # for the wheel
                index = 1
                scale = self.sensor_scale[index]
                values = [float(x) * scale for x in tokens[1:4]]
                self.sensor[index] = values
                self.sensor[self.sensor_map[index]] = values
            if tokens[0] == '-S16':  # for the servos positions
                   index = 7
                   scale = self.sensor_scale[index]
                   values = [float(x) * scale for x in tokens[4:9]]
                   self.sensor[index] = values
                   self.sensor[self.sensor_map[index]] = values
            if tokens[0] == '-S14':  # for the servos load
                index = 9
                scale = self.sensor_scale[index]
                values = [float(x) * scale for x in tokens[4:9]]
                self.sensor[index] = values
